...
pos = radius*np.random.uniform(-1, 1, size=(N,2))
for i in range(0,N):
    value = np.sqrt(pos[i,0]**2 + pos[i,1]**2)
    while value > radius:
        pos[i,:] = radius*np.random.uniform(-1,1,size=(1,2))
        value = np.sqrt(pos[i,0]**2 + pos[i,1]**2)
pos_z = np.random.uniform(-1,1,size=(N,1))
...
p = ax.scatter(pos[:,0], pos[:,1], pos_z, '.', c = mass[:,0])
