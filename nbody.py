...
# Leapfrog integrator
def leapfrog(derivs, froggy, dt, n, N, m):
    # froggy = np.zeros(shape=(N, 6, n))
    froggy = np.zeros(shape=(N, 7, n))
    # froggy[:, :, 0] = y[:, 0:6]
    froggy[:, 1:7, 0] = y[:, 0:6]
    # ms = np.empty(shape=(N, 1, n))
    # ms[:, 0, 0] = mass[:]
    froggy[:, 0, 0] = mass[:]
    count = 1
    for i in range(1, n):
        # ms[:, 0, i] = mass[:]
        froggy[:, 0, i] = mass[:]
        # half = froggy[:, 0:3, i - 1] + 0.5 * dt * froggy[:, 3:6, i - 1]
        half = froggy[:, 1:4, i - 1] + 0.5 * dt * froggy[:, 4:7, i - 1]
        froggy[:, 4:7, i] = froggy[:, 4:7, i - 1] + dt * derivs(N, half, froggy[:, 4:7, i - 1], epsil2, mass_array)[:, 4:7]
...
trial = leapfrog(derivs, y, h, n, N, mass_array)
